...
export default function OverviewSection({ onNavigate, latestRun }: OverviewSectionProps) {
  ...
  return (
    <div className="p-8 space-y-8">
      {/* Quick Stats banner — shows latest processed run */}
      {latestRun && (
            ...
            {recentPapers.map((paper) => (
              <div key={paper.id} className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">{paper.year} · {paper.authors[0]}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {mockTopics.map((topic) => (
            <div key={topic.id} className="p-4 rounded-xl border border-gray-100" style={{ borderLeftColor: topic.color, borderLeftWidth: 3 }}>
              ...
              <div className="flex items-center gap-1 mt-2">
                <div className="flex-1 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ backgroundColor: topic.color, width: `${(topic.paperIds.length / mockPapers.length) * 100 * 2.5}%` }}
                  />
                </div>
                <span
